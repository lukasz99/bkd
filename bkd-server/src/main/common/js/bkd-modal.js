BKDmodal = {
  init: function( anchor, showAnchor, url ){
    ...
    $( showAnchor ).on( 'click', { 'anchor':anchor, 'url': url },
      function (event) {
        ...
        $( event.data.anchor ).append("<table class='bkd-modal-header' width='100%'>"+
           "<tr>"+
        ...
        $(event.data.anchor + " .bkd-modal-frame").css( 'width', hwdth-250-hbwl-hbwr);       
        $(event.data.anchor + " .bkd-modal-frame").height( fpos.top-hhght-75-50);

        $( event.data.anchor + " .bkd-modal-close" ).on( 'click', {'anchor':event.data.anchor},
          function (ev) {
                $( ev.data.anchor ).hide();
