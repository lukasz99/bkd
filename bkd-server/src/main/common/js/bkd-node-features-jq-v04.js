...
BKDnodeFeatures = {
    ...
    vclass: [ {"id":"flist-select-1","name":"select-1","label":"Benign", "value":"benign", "color":"blue" },
              ...
              {"id":"flist-select-3","name":"select-3","label":"Conflicting Evidence", "value":"conflicting evidence", "color":"#cc79de" },
    ...
    init: function( tgt, format, data, myurl ){
        ...
        console.log( "Features -> detailtable: " + JSON.stringify( this.config.lollipanel.detailtable ) );
        ...
        for( var i =0; i < data.attrs.length;i++ ){
            var catt = data.attrs[i];            
            if( seqVarList.includes(catt.cvType.name) ){                 
                var cseq = { upr: '', seq: catt.value, xref:[] };
                
                cseq.mane = (catt.cvType.name == 'mane-sequence'); //  ? true : false;
                cseq.canon = (catt.cvType.name == 'canonical-sequence'); //  ? true : false;
                if( catt.xrefs != undefined ){
                    for( var j=0; j< catt.xrefs.length; j++){
                        cseq.xref.push( catt.xrefs[j] );
                        if( catt.xrefs[j].ns =='upr')
                            cseq.upr = catt.xrefs[j].ac;
                    }
                }
                iseq.push( cseq );
                 ...
                 function( event ){
                     ...
                     var nloli =
                         new BkdLollipop( { anchor: "flist-lollipop-1",
                                            id: "lpanel-1", 
                                            url: parent.myurl.replace("FULL","FEATL"),
                     ...
                     nloli.initialize ( { sequence: cseq });                     
                 });
        ...
        $('#snflag').click(function(){
            ...
            */
            ...
            console.log("SNFlag toggle");
            
            BKDnodeFeatures.lollipanels['loli1']
                .state.datasrc.snflag = $('#snflag').is(':checked');
        ...
        console.log("BKDnodeFeatures.init(myurl): ",  myurl);
        ...
        var loli1 = new BkdLollipop( { anchor: "flist-lollipop-1",
                                       ...
                                       vclass: BKDnodeFeatures.vcalss,
                                       detailtable: this.config.lollipanel.detailtable  
                                     } );
        
        loli1.initialize ( { sequence: BKDnodeFeatures.data.sequence });
    ...
    lollipanel: function( data ){

        var config = this.config.lollipanel;
        ...
        console.log("lolipanel: init");
        console.log("iseq:", $("#iseq").val());
                        ...
                        if( col['xref-ns'] == null ){
                            ...
                            if( cns == feat.xref[j].ns || cns == feat.xref[j].ns.toLowerCase() ){
                                ...
                                if( 'string' == col.type ){
                                    ...
                                    var href = "<a href='"+clink+"' target='_blank'>"+ctext+"</a>";
                                    dtval[cid].push( href );
                                }
                            } 
                        }
        ...
        lollipop.options.lollipopHook =
            { action: BKDnodeFeatures.buildFDets,
              options: { anchor: config.detailtable.anchor,
    ...
    fselcontrol: function( anchor, clist, lpanel, action){
