...
D3MSA = {
        ...
        $.ajax( { url: this.msaurl,
            ...
            .done( function( data, textStatus, jqXHR ){
               ...
               // brush range corresponding to longest aa step (aaMaxStep)
                ...
                var brlimit = D3MSA._msaW/D3MSA.config.aaMaxStep;
                brlimit = brlimit/D3MSA.msaSeq[0].length*D3MSA._msaW; 
                
                if( brlimit >D3MSA.config.brushLimit){  // no slope modification                  
    ...
    navView: function(){
        ...
        _left = D3MSA.navButton(D3MSA.svg, "msaleft", 5, 13, "<")
            .on( "click", (event) => {                
                D3MSA.setNavBrush( 2*D3MSA.brushLeft-D3MSA.brushRight,
                                   D3MSA.brushLeft);                
        ...
        _right = D3MSA.navButton(D3MSA.svg, "msaright", 73, 13, ">")
            .on( "click", (event) => {
                D3MSA.setNavBrush( D3MSA.brushRight,
                                   2*D3MSA.brushRight-D3MSA.brushLeft);                
                D3MSA.navBrushG.call( D3MSA.viewport.move,
                                      [D3MSA.brushLeft, D3MSA.brushRight]);
            });        
    ...
    getViewportParams(){
        ...
        var windowWidth = ( D3MSA.displayend - D3MSA.displaystart)
            / 100 * (D3MSA.msaRange[1]);   // [AA]
        
        var windowCenter = 0.005 * ( D3MSA.displayend + D3MSA.displaystart )
            * (D3MSA.msaRange[1]) ;        
        ...
        if( D3MSA.displayend - D3MSA.displaystart <50 ){        
            D3MSA.alpha=(D3MSA.msaRange[1]/2-D3MSA._msaW/D3MSA.config.aaMaxStep)
                / 100 / (1 - D3MSA._brl/D3MSA._msaW - 0.5);
            
            D3MSA.beta = D3MSA.msaRange[1]/2 - 50 * D3MSA.alpha;

            D3MSA.ww = D3MSA.alpha * ( D3MSA.displayend - D3MSA.displaystart)
                + D3MSA.beta;
        ...
        if( D3MSA.displaystart < 100 - D3MSA.displayend ) {
            D3MSA.aaOffset = + D3MSA.aaStep * D3MSA.displaystart/100
                *D3MSA.msaRange[1];
        } else {
            D3MSA.aaOffset = - D3MSA._msaW + D3MSA.aaStep * D3MSA.displayend/100
                *D3MSA.msaRange[1];            
        }
    ...
    renderRange() {        
        ...
        var opq = Math.min( 1, 1 - D3MSA.aaStep**1.5
                            / (D3MSA.config.aaMaxStep - D3MSA.aaStep*0.99)**2);
        ...
        for( var i = 0; i <D3MSA.msaHead.length; i++ ){
            ...
            d3.select( "#seqSeq" + i.toString() +"rn")
                ...
                .attr( "x", pos )
                .attr( "y", -10 )
                .attr( "width", (D3MSA.seqRng[i]["max"]
                                 + D3MSA.aaStep
                                 - D3MSA.seqRng[i]["min"])*D3MSA.aaStep)
                .attr( "height", 10 );
    ...
    updateHeadMSA: function(){
        ...
        D3MSA.msaMin = new Array(D3MSA.msaHead.length);   // one per sequence
        D3MSA.msaMax = new Array(D3MSA.msaHead.length);  

        D3MSA.msaPos = new Array(D3MSA.msaSeq[0].length);     // one per aa
        D3MSA.rectPos = new Array(D3MSA.msaSeq[0].length);

        var jmin= parseInt(D3MSA.aaOffset/D3MSA.aaStep + 1/2);
        var jmax= parseInt((D3MSA._msaW+D3MSA.aaOffset)/D3MSA.aaStep - 1/2);
        if( jmin < 0 ) jmin = 0;
        if( jmax >= D3MSA.msaSeq[0].length ) jmax = D3MSA.msaMap[i].length-1;

        var frMsaPos = new Array(jmax-jmin+1);
        var frRectPos = new Array(jmax-jmin+1);
        
        D3MSA.msaNewFrame = {"min":jmin,
                             "max":jmax,
                             "msaMin": D3MSA.msaMin,
                             "msaMax": D3MSA.msaMax,
                             "msaPos": frMsaPos,
        ...
        for( var i=0; i <D3MSA.msaHead.length; i++ ){                
            D3MSA.msaMin[i] = D3MSA.msaMap[i][jmin];
            D3MSA.msaMax[i] = D3MSA.msaMap[i][jmax];
        }

        for( var j = 0; j <D3MSA.msaPos.length; j++ ){   // j - sequence position
            D3MSA.msaPos[j] = -100;
            D3MSA.rectPos[j] = -100;
        }
        ...
        for( var j = jmin; j <=jmax; j++ ){   // j - sequence position
            ...
            frMsaPos[j-jmin]= D3MSA.msaPos[j];
            frRectPos[j-jmin]= D3MSA.rectPos[j];            
        }
    ...
    renderHeadMSA: function(){
        ...
        for( var j = 0; j < D3MSA.msaSeq[0].length; j++ ){   // j - seq pos
            var pos = 0.5 * D3MSA.aaStep + j * D3MSA.aaStep - D3MSA.aaOffset;
            ...
            try{
                ...
                if( pos > D3MSA._msaW-D3MSA.aaStep && pos < D3MSA._msaW ){
                    ...
                    for( var i=0; i <D3MSA.msaHead.length; i++ ){
                        ...
                        if( j >= D3MSA.msaMap[i].length ){
                            D3MSA.msaMax[ i ]
                                = D3MSA.msaMap[i][ D3MSA.msaMap[i].length-1 ];
                        } else {
                            D3MSA.msaMax[ i ]
                                = D3MSA.msaMap[i][j];
                        }
    ...
    renderAA() {
        ...
        for( var i = 0; i < D3MSA.rectPos.length; i++ ){
            D3MSA.msaVon.push( D3MSA.rectPos[i] < -D3MSA.aaStep ||
                               D3MSA.rectPos[i] > D3MSA._msaW  ? false : true);
        }

        for( var i = 0; i < D3MSA.msaPos.length; i++ ){
            if( D3MSA.msaVon[i] ){  // add AAs
                for( var j = 0; j < D3MSA.msaHead.length; j++ ){
                    if( D3MSA.rectPosOld[i] <=-10
                        || D3MSA.rectPosOld[i] > D3MSA._msaW) continue;
                    // box
    ...
    updateAA() {
        ...
        //console.log(D3MSA.msaNewFrame);
        //console.log(":"+D3MSA.aaStep+":"+D3MSA.aaOffset+":");
        
        for( var i = 0; i < D3MSA.rectPos.length; i++ ){
            msaVonCurrent.push( D3MSA.rectPos[i] < -D3MSA.aaStep  ||
                                D3MSA.rectPos[i] > D3MSA._msaW  ? false:true);        
        }

        for( var i = 0; i < D3MSA.rectPos.length; i++ ){
            if( oonmin < 0 &&  D3MSA.msaVon[i]) oonmin = i;
            if(  D3MSA.msaVon[i] ) oonmax = i;
        }

        var smin = D3MSA.msaNewFrame.min;
        var joff = D3MSA.msaNewFrame.min;
        
        if( oonmin < smin){
            smin = oonmin;
            joff = oonmin;
        }
        var smax = D3MSA.msaNewFrame.max;
        ...
        for( j = smin; j <= smax; j++ ){
            ...
            cur = (j >= D3MSA.msaNewFrame.min &&
                   j <= D3MSA.msaNewFrame.max) ? true : false;
            ...
            if( cur != old ){ 
                if( cur ){ // add AA/box
                    for( var i = 0; i < D3MSA.msaHead.length; i++ ){
                        ...
                        var aa =  D3MSA.msaSeq[i][j];
                        
                        d3.select( "#seqSeq" + i.toString() +"bk")  // box
                            .append( "rect" )                
                            .attr( "i", j )
                            .attr( "x", D3MSA.rectPosOld[j] )
                            //.attr( "x", D3MSA.msaNewFrame.rectPos[j] )
                            .attr( "y", -10 )
                            .attr( "width", D3MSA.aaStepOld )
                            .attr( "height", 10 )
                            .attr( "class", "msa-rect")
                            .style( "fill", D3MSA._palette1[ aa ] )
                            .attr( "fill-opacity", D3MSA.aaStep > 16 ? 0.5:1 )
                            .append("title").text(aa+(D3MSA.msaMap[i][j+joff]));
                        ...
                        d3.select( "#seqSeq" + i.toString() +"aa") // letter 
                            ...
                            .style( "fill", D3MSA._palette1[ aa ] )
                            .attr( "text-anchor", "middle") 
                            .attr( "x", D3MSA.msaPosOld[j] )
                            .attr( "y", 0 )
                            .attr( "i", j )
                            .attr( "fill-opacity", D3MSA.aaStep > 16 ? 1 : 0 )
                            .text( aa )
                            .append("title").text(aa + (D3MSA.msaMap[i][j]));
                    }                        
        ...
        for( var i = 0; i <D3MSA.msaHead.length; i++ ){
            for( var j = 0; j < D3MSA.msaNewFrame.msaPos.length; j++){
                
                d3.select( "#seqSeq" + i.toString() +"bk " +
                           "rect[i='"+(j+D3MSA.msaNewFrame.min)+"']")
                    .attr( "fill-opacity", D3MSA.msaOpa)
                    .attr( "x", D3MSA.msaNewFrame.rectPos[j] )
                    .attr( "width", D3MSA.aaStep );                    
            }
        ...
        for( var i = 0; i <D3MSA.msaHead.length; i++ ){
            for( var j = 0; j < D3MSA.msaNewFrame.msaPos.length; j++){            
                d3.select( "#seqSeq" + i.toString() +"aa " +
                           " text[i='"+(j+D3MSA.msaNewFrame.min) + "']")
                    .attr( "x", D3MSA.msaNewFrame.msaPos[j])
                    .attr( "fill-opacity", function (d) {
                        if( D3MSA.aaStep > 16 ){
                            return 1;
                        } else {
    ...
    setSelectMap: function( smap ){
        //console.log(smap);
        for( pc of D3MSA.select ){ // go over existing elements
            //console.log("sel:",pc);
        ...
        for( pc of Object.keys(smap) ){
            ...
            pcl =pc.split(":");
            D3MSA.select[pc] =  D3MSA.addSelect( Number(pcl[0]),
                                                 pcl[1],
                                                 smap[pc].name);            
        }
    },
    ...
    setSelectList: function( slist ){
        ...
        for( i = 0 ; i < slist.length; i++){
            pcl = slist[i].split(":");
            D3MSA.select[ slist[i] ] =  D3MSA.addSelect(Number(pcl[0]),pcl[1],pcl[0]);            
        }
