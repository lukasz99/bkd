console.log("bkd-msaview: common");
...
class BkdViewMSA {
    ...
    constructor( config ){
        this._conf = {   // defaults
            ...
            _palette1: { "A":"green", "C":"orange", "D":"red",   "E":"red",
                         "F":"purple","G":"cyan",  "H":"blue",  "I":"green",
                         "K":"blue",  "L":"green",  "M":"green", "N":"grey",
                         "P":"cyan", "Q":"grey",   "R":"blue",  "S":"grey",
                         "T":"grey",  "V":"green",  "W":"purple", "Y":"purple",
                         "-":"white", "~": "white",  ".":"white" },
            
            _palette2: { "A":"green", "C":"grey", "D":"white", "E":"white",
                         "F":"purple", "G":"grey",  "H":"white", "I":"green",
                         "K":"white", "L":"green",  "M":"green", "N":"grey",
                         "P":"grey", "Q":"grey",   "R":"white",  "S":"grey",
                         "T":"grey",  "V":"green",  "W":"purple", "Y":"purple",
                         "-":"white", "~": "white", ".":"white" },
            msaDY: 20,
            msaBoxY: 16,
    ...
    initialize( init ){
        ...
        if( init.dtrac != undefined){
            this._data.dtrac = init.dtrac;
        }

        console.log("BkdViewMSA.initialize: this._data.dtrac -> ", this._data.dtrac ); 
        
        this.mymsa = new BkdMSA( {myurl: this._data.url,
                                  dtrac: this._data.dtrac } );
        

        console.log( "BkdViewMSA.initialize: this.mymsa.data.dtrac -> ",
                     this.mymsa.data.dtrac );
        ...
        var callback = function( msa ){
            return function( data, textStatus, jqXHR) {
               ...
               // brush range corresponding to longest aa step (aaMaxStep)
                ...
                d3.select( "#" + msa._view.anchor.replace("#","") )
    ...
    getTgtSeqSelStr( nseq, ntgt=0 ){ 
        return this.mymsa.getTgtSeqSelStr( nseq, ntgt);  // mymsa
    }

    getTgtSeqSelStrByID( selID, tgtID ){ 
        return this.mymsa.getTgtSeqSelStrByID( selID, tgtID );  // mymsa
    }
    
    getMsaSeqSel( nseq=0 ){  // MSA ranges with non-empty nseq
        return this.mymsa.getMsaSeqSel( nseq);  // mymsa
    }
    
    getSeqPos( pos, ref ){   
        // sequence positions corresponding to pos in ref sequence 
        return this.mymsa.getSeqPos( pos, ref );
    }
    
    getEnt(){        
        return this.mymsa.getEnt();
    }
    
    getVCnt(){
        return this.mymsa.getVCnt();
    }

    getMSA(){
        return this.mymsa.getMSA();
    }
    ...
    _initViz(){
        ...
        this._view.msaHeight = msaHeight;

        this._view.svgHeight = msaHeight;
    ...
    _render(){
        ...
        //.domain( this._data.msaRange )
            .domain( D.msaRange )
        //.range([0, this._data.msaSeq[0].length-1]);  // mymsa
            .range([0, D.msaSeq[0].length-1]);          
        ...
        for( var t in C.tlist ){
            if( C.tlist[t] == 'slogo' ){
                ...
                if( 'dtrac' in D && D.dtrac.length > 0 ){
                    this.renderDtrac();
                }
            }
        }                
    ...
    renderMSA(){
        ...
        for( var i = 0; i < this._data.msaSeq.length; i++){
            ...
            //.push(this._data.msaMap[i][this._data.msaSeq[i].length-1]);
                .push(D.msaMap[i][D.msaSeq[i].length-1]);
        }
        ...
        //.attr("height", this._conf.msaDY*(this._data.msaHead.length+1));
            .attr("height", this._conf.msaDY*(D.msaHead.length+1));
    ...
    updateHeadMSA(){
        ...
        this._view.msaMin = new Array(D.msaHead.length);  
        this._view.msaMax = new Array(D.msaHead.length);  

        // one per aa
        //this._view.msaPos = new Array(this._data.msaSeq[0].length);     
        //this._view.rectPos = new Array(this._data.msaSeq[0].length);

        this._view.msaPos = new Array(D.msaSeq[0].length);     
        this._view.rectPos = new Array(D.msaSeq[0].length);
        ...
        if( jmax >= D.msaSeq[0].length )
            jmax = D.msaSeq[0].length-1;
        ...
        for( var i=0; i <this._data.msaHead.length; i++ ){                
            ...
            this._view.msaMin[i] = D.msaMap[i][jmin];
            this._view.msaMax[i] = D.msaMap[i][jmax];
        }
    ...
    renderDtrac(){
        ...
        if( D.dtrac == undefined || D.dtrac.length == 0) return;  
    ...
    setSelectList( slist, rseq, rcol, cseq, ccol, poi ){
        ...
        console.debug("setSelectList:",sindex, D.msaHead[sindex][rcol]);
        
        var smap = D.msaMap[ sindex ]; //         
        ...
        for( var i = 0 ; i < slist.length; i++){
            ...
            if( cseq !== undefined && ccol != undefined ){
                ...
                var msapos = D.msaRMap[sindex][Number(pcl[0])];
                var canpos = D.msaMap[0][msapos];
    ...
    updateSelect(){
        ...
        for( var k in this._view.select ){
            var kl = k.split(":");
            console.log(kl);
            var mp = D.msaRMap[sindex][ Number( kl[0] ) ];
    ...
    updatePOI(){
        ...
        for( var k in this._view.poi.pos ){
            ...
            var mp = D.msaRMap[spoi][ Number( ppoi ) ];
            var pos = mp * this._view.aaStep - this._view.aaOffset;
    ...
    getMsaPos(sindex,pos){
        ...
        var msaMap = D.msaMap[sindex];  // msaMap[msa]=seq
        var lb = 0;
