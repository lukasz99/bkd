            ...
            .done( function( data, textStatus, jqXHR ){
                ...
                for( var i=0; i < lines.length; i++ ){
                    if( lines[i].startsWith('>') ){
                        ...
                        D3MSA3.msaHead.push( hcols );
                        D3MSA3.msaSeq.push("");
                    } else {
                        D3MSA3.msaSeq[D3MSA3.msaSeq.length-1]+= lines[i];
                    }
                }
                D3MSA3._brl = D3MSA3.config.brushLimit;    // shortest brush range
                
               // brush range corresponding to longest aa step (aaMaxStep)
                
                var brlimit = D3MSA3._msaW/D3MSA3.config.aaMaxStep;
                brlimit = brlimit/D3MSA3.msaSeq[0].length*D3MSA3._msaW; 
                
                if( brlimit >D3MSA3.config.brushLimit){  // no slope modification                  
                    D3MSA3._brl = Math.floor( brlimit );
                    
                } 
                                
                //render                
                setTimeout( D3MSA3.render(), 1000 );
...
D3MSA3x = {
    ...
    render: function(){
        ...
        for( var i=0; i <  D3MSA3.msaSeq.length; i++ ){
            D3MSA3.msaMap.push([]);
            D3MSA3.seqRng.push({"min":-1,"max":-1});
            var cp = 0;
            for( var j=0; j < D3MSA3.msaSeq[i].length; j ++ ){
                if( "ACDEFGHIKLMNPQRSTVWY".includes( D3MSA3.msaSeq[i][j] ) ){
                    cp +=1;
                    if( i==0 ){
                        D3MSA3.msaRMap[cp] = j;
                    }
                    ...
                    if(D3MSA3.seqRng[D3MSA3.seqRng.length-1]["min"]<0){
                        D3MSA3.seqRng[D3MSA3.seqRng.length-1]["min"]=j;
                    }
                    D3MSA3.seqRng[D3MSA3.seqRng.length-1]["max"]=j;
                }
                
                if( cp >0 ){
                    D3MSA3.msaMap[i].push( cp );
                } else {
                    D3MSA3.msaMap[i].push( "" );
                }
            }
        }
        ...
        D3MSA3.msaRange = [0, D3MSA3.msaSeq[0].length];

        D3MSA3.msaScale = d3.scaleLinear()  //  1 -> 1
            .domain(D3MSA3.msaRange)
            .range([0, D3MSA3.msaSeq[0].length-1]);
        
        D3MSA3.navScale = d3.scaleLinear()  //  100 ->  pixelWidth
            .domain([0, 100])
            .range([0, D3MSA3._msaW]);
        
        D3MSA3.nav2msaScale = d3.scaleLinear()  // 100 -> maxAA
            .domain([0, 100])
            .range(D3MSA3.msaRange);        
    ...
    navView: function(){
        ...
        D3MSA3.displaystart = 0;
        D3MSA3.displayend =  100;

        _left = D3MSA3.navButton(D3MSA3.svg, "msaleft", 5, 13, "<")
            .on( "click", (event) => {                
                D3MSA3.setNavBrush( 2*D3MSA3.brushLeft-D3MSA3.brushRight,
                                   D3MSA3.brushLeft);                
                D3MSA3.navBrushG.call( D3MSA3.viewport.move,
        ...
        _plus = D3MSA3.navButton(D3MSA3.svg, "msaplus", 27, 13, "+")
            .on( "click", (event) => {
                var center = (D3MSA3.brushLeft + D3MSA3.brushRight)/2;
                var delta = 0.75 * (D3MSA3.brushRight - D3MSA3.brushLeft)/2;
                D3MSA3.setNavBrush( center-delta, center+delta);                
        ...
        _minus = D3MSA3.navButton(D3MSA3.svg, "msaminus", 50, 13, "-", -1 )
            .on( "click", (event) => {
                var center = (D3MSA3.brushLeft + D3MSA3.brushRight)/2;
                var delta = 1.5 * (D3MSA3.brushRight - D3MSA3.brushLeft)/2;
                D3MSA3.setNavBrush( center-delta, center+delta);                
        ...
        _right = D3MSA3.navButton(D3MSA3.svg, "msaright", 73, 13, ">")
            .on( "click", (event) => {
                D3MSA3.setNavBrush( D3MSA3.brushRight,
                                   2*D3MSA3.brushRight-D3MSA3.brushLeft);                
                D3MSA3.navBrushG.call( D3MSA3.viewport.move,
                                      [D3MSA3.brushLeft, D3MSA3.brushRight]);
            });        


        _full =  D3MSA3.navButton(D3MSA3.svg, "msafull", 560, 13, "Full")
            .on( "click", (event) => {
                D3MSA3.setNavBrush( 0, D3MSA3._msaW);                
                D3MSA3.navBrushG.call( D3MSA3.viewport.move,
                                      [D3MSA3.brushLeft, D3MSA3.brushRight])
        ...
        _navG = D3MSA3.svg.append("g").attr( "id", "msanav" )
            .attr("transform", "translate(100, 10)");

        _msaAxis = d3.axisBottom( D3MSA3.navScale );
        _msaAxis.ticks( 10 ).tickFormat('');
                
        D3MSA3.navAxis = _navG.append('g')
            .attr('class', 'x axis')
            .call(_msaAxis);

        
        D3MSA3.brushLeft = 0;
        D3MSA3.displaystart = D3MSA3.navScale.invert( 0 );
        
        D3MSA3.brushRight = D3MSA3._brl;       
        D3MSA3.displayend = D3MSA3.navScale.invert( D3MSA3._brl );
        
        D3MSA3.viewport = d3.brushX()
            .handleSize(6)
            .extent([ [0, 0], [D3MSA3._msaW, 10] ])            
            .on( "brush", (event) => {
                
                D3MSA3.setNavBrush( event.selection[0], event.selection[1]);

                D3MSA3.displaystart = D3MSA3.brushLeft/D3MSA3._msaW*100;
                D3MSA3.displayend = D3MSA3.brushRight/D3MSA3._msaW*100;
                                                
                D3MSA3.updatePolygon();
                D3MSA3.updateMSA();              
                D3MSA3.updateSelect();                
            });
                
        D3MSA3.navBrushG = _navG.append("g")
            .attr("class", "brush")
            .call(D3MSA3.viewport);
        
        D3MSA3.navBrushG .call( D3MSA3.viewport.move, [ 0, D3MSA3._brl ] );
        
        D3MSA3.polygon = _navG.append("polygon")
            .attr('class', 'zoom-polygon')
            .attr('fill', '#777')
            .attr('fill-opacity','0.3');
        
        D3MSA3.updateNavRuler();   
    },
    
    setNavView: function( center, width ){
        
        D3MSA3.setNavBrush( center-width/2, center+width/2 );                
        D3MSA3.navBrushG.call( D3MSA3.viewport.move,
                              [D3MSA3.brushLeft, D3MSA3.brushRight] );
    },
    ...
    setNavBrush: function( left, right ){
        ...
        if( right - left <  D3MSA3._brl ){           
            brushCenter = 0.5 * (left + right);
            brushLeft = brushCenter - D3MSA3._brl/2;
            brushRight = brushCenter + D3MSA3._brl/2;
            
            if( brushLeft < 1 ){
                brushLeft  = 0;
                brushRight = D3MSA3._brl;  // /D3MSA3._brRatio;
            }

            if( brushRight >  D3MSA3._msaW ){ 
                brushLeft = D3MSA3._msaW - D3MSA3._brl - 1; // /D3MSA3._brRatio - 1;
                brushRight = D3MSA3._msaW ;                      
            }
        ...
        D3MSA3.brushLeft = brushLeft;
        D3MSA3.brushRight = brushRight;        
    },


    getViewportParams(){

        var windowWidth = ( D3MSA3.displayend - D3MSA3.displaystart)
            / 100 * (D3MSA3.msaRange[1]);   // [AA]
        
        var windowCenter = 0.005 * ( D3MSA3.displayend + D3MSA3.displaystart )
            * (D3MSA3.msaRange[1]) ;        
        
        D3MSA3.aaStep   =  ( D3MSA3._msaW ) / windowWidth;  // [pixel/aa]
        D3MSA3.aaOffset =  (windowCenter) * D3MSA3.aaStep - D3MSA3._msaW/2;

        D3MSA3.windowWidth = windowWidth;
        D3MSA3.windowCenter = windowCenter;

        if( D3MSA3.displayend - D3MSA3.displaystart <50 ){        
            D3MSA3.alpha=(D3MSA3.msaRange[1]/2-D3MSA3._msaW/D3MSA3.config.aaMaxStep)
                / 100 / (1 - D3MSA3._brl/D3MSA3._msaW - 0.5);
            
            D3MSA3.beta = D3MSA3.msaRange[1]/2 - 50 * D3MSA3.alpha;

            D3MSA3.ww = D3MSA3.alpha * ( D3MSA3.displayend - D3MSA3.displaystart)
                + D3MSA3.beta;

            D3MSA3.aaStep =  D3MSA3._msaW /D3MSA3.ww ;
            D3MSA3.aaOffset =  (windowCenter) * D3MSA3.aaStep - D3MSA3._msaW/2;
        }

        D3MSA3.msaOpa = 0.5;
        if( D3MSA3.aaStep <= 16 ){
            D3MSA3.msaOpa = 0.5 + 0.5* (16-D3MSA3.aaStep)/16;
        }

        if( D3MSA3.displaystart < 100 - D3MSA3.displayend ) {
            D3MSA3.aaOffset = + D3MSA3.aaStep * D3MSA3.displaystart/100
                *D3MSA3.msaRange[1];
        } else {
            D3MSA3.aaOffset = - D3MSA3._msaW + D3MSA3.aaStep * D3MSA3.displayend/100
                *D3MSA3.msaRange[1];            
        }
    ...
    updatePolygon() {
        ...
        if (D3MSA3.polygon)
            D3MSA3.polygon.attr(
                "points",
                `${D3MSA3.navScale(D3MSA3.displaystart)},10
        ${D3MSA3.navScale(D3MSA3.displayend)},10
        ${D3MSA3._msaW},25
        0,25`
            );
    },
    ...
    renderMSA: function(){
        D3MSA3.getViewportParams();

        D3MSA3.msaVon = [];        
        D3MSA3.msaMin = [];  // one per sequence
        D3MSA3.msaMax = [];
        
        D3MSA3.msaPos = [];  // common to all seqs
        D3MSA3.rectPos = [];

        for( var i = 0; i < D3MSA3.rectPos.length; i++ ){
            D3MSA3.msaVon.push(false);
        }
        
        for( var i = 0; i < D3MSA3.msaSeq.length; i++){
            D3MSA3.msaMin.push(1);
            D3MSA3.msaMax.push(D3MSA3.msaMap[i][D3MSA3.msaSeq[i].length-1]);
        }
        
        D3MSA3.offsetY = 10;
        if( D3MSA3.config.navig ) D3MSA3.offsetY += 30;
        if( D3MSA3.config.slogo ) D3MSA3.offsetY += 50;
        
        var _seqMSA = D3MSA3.svg.append("g").attr( "id", "seqMSA" )
            .attr("transform", "translate(0, " + ( D3MSA3.offsetY) + ")");        
        _seqMSA.append("clipPath")
            .attr("id", "seqMSA_clip")
            .append("rect")
            .attr("x", 0)
            .attr("y", -6)
            .attr("width", D3MSA3._msaW) // TODO
            .attr("height", 14*(D3MSA3.msaHead.length+1));
        ...
        D3MSA3.renderHeadMSA();
        
        if( D3MSA3.aaStep >= D3MSA3.config.aaMinStep ){
            //D3MSA3.updateAA();
            D3MSA3.renderAA();
        }
        
        if( D3MSA3.aaStep < D3MSA3.config.aaMinStep*5 ){            
            D3MSA3.renderRange();
            D3MSA3.rngOn = true;
        }
        
        D3MSA3.setSelectList( [], null  );  // set to empty selection        
    },

    updateMSA() {      // called on bush move
        D3MSA3.getViewportParams();
        D3MSA3.updateHeadMSA();
    ...
    renderRange() {        
        ...
        var opq = Math.min( 1, 1 - D3MSA3.aaStep**1.5
                            / (D3MSA3.config.aaMaxStep - D3MSA3.aaStep*0.99)**2);

        for( var i = 0; i <D3MSA3.msaHead.length; i++ ){
            
            var pos =  D3MSA3.seqRng[i]["min"] * D3MSA3.aaStep - D3MSA3.aaOffset;
            d3.select( "#seqSeq" + i.toString() +"rn")
                ...
                .attr( "x", pos )
                .attr( "y", -10 )
                .attr( "width", (D3MSA3.seqRng[i]["max"]
                                 + D3MSA3.aaStep
                                 - D3MSA3.seqRng[i]["min"])*D3MSA3.aaStep)
                .attr( "height", 10 );
    ...
    updateHeadMSA: function(){
        ...
        D3MSA3.msaPosOld = D3MSA3.msaPos;
        D3MSA3.rectPosOld = D3MSA3.rectPos;
        D3MSA3.aaStepOld = D3MSA3.aaStep;
                
        D3MSA3.msaMin = new Array(D3MSA3.msaHead.length);   // one per sequence
        D3MSA3.msaMax = new Array(D3MSA3.msaHead.length);  

        D3MSA3.msaPos = new Array(D3MSA3.msaSeq[0].length);     // one per aa
        D3MSA3.rectPos = new Array(D3MSA3.msaSeq[0].length);

        var jmin= parseInt(D3MSA3.aaOffset/D3MSA3.aaStep + 1/2);
        var jmax= parseInt((D3MSA3._msaW+D3MSA3.aaOffset)/D3MSA3.aaStep - 1/2);
        if( jmin < 0 ) jmin = 0;
        if( jmax >= D3MSA3.msaSeq[0].length ) jmax = D3MSA3.msaMap[i].length-1;

        var frMsaPos = new Array(jmax-jmin+1);
        var frRectPos = new Array(jmax-jmin+1);
        
        D3MSA3.msaNewFrame = {"min":jmin,
                             "max":jmax,
                             "msaMin": D3MSA3.msaMin,
                             "msaMax": D3MSA3.msaMax,
                             "msaPos": frMsaPos,
        ...
        for( var i=0; i <D3MSA3.msaHead.length; i++ ){                
            D3MSA3.msaMin[i] = D3MSA3.msaMap[i][jmin];
            D3MSA3.msaMax[i] = D3MSA3.msaMap[i][jmax];
        }

        for( var j = 0; j <D3MSA3.msaPos.length; j++ ){   // j - sequence position
            D3MSA3.msaPos[j] = -100;
            D3MSA3.rectPos[j] = -100;
        }
        
        for( var j = jmin; j <=jmax; j++ ){   // j - sequence position
            var pos = 0.5 * D3MSA3.aaStep + j * D3MSA3.aaStep - D3MSA3.aaOffset;
            D3MSA3.msaPos[j] = pos;
            D3MSA3.rectPos[j] = pos - 0.5 * D3MSA3.aaStep;

            frMsaPos[j-jmin]= D3MSA3.msaPos[j];
            frRectPos[j-jmin]= D3MSA3.rectPos[j];            
        }

        for( var i = 0; i <D3MSA3.msaHead.length; i++ ){                     
            if(D3MSA3.msaMin){                
                d3.select( "#seqHead" + i.toString() + " .msa-min")
                    .text( D3MSA3.msaMin[i] );
                }
            
            if(D3MSA3.msaMax){
                d3.select( "#seqHead" + i.toString() + " .msa-max")
                    .text( D3MSA3.msaMax[i] );
            }                       
        }
    },
    ...
    renderHeadMSA: function(){
        ...
        D3MSA3.msaMin = [];     // one per sequence
        D3MSA3.msaMax = [];  
        D3MSA3.msaPos = [];     // one per aa
        D3MSA3.rectPos = [];

        for( var j = 0; j < D3MSA3.msaSeq[0].length; j++ ){   // j - seq pos
            var pos = 0.5 * D3MSA3.aaStep + j * D3MSA3.aaStep - D3MSA3.aaOffset;
            
            try{
                if( pos > 0 && pos < D3MSA3.aaStep){   // first AA visible
                    for( var i=0; i <D3MSA3.msaHead.length; i++ ){                
                        D3MSA3.msaMin.push(D3MSA3.msaMap[i][j]);
                    }
                }
            
                if( pos > D3MSA3._msaW-D3MSA3.aaStep && pos < D3MSA3._msaW ){

                    // last AA visible

                    for( var i=0; i <D3MSA3.msaHead.length; i++ ){
                        D3MSA3.msaMax.push( D3MSA3.msaMap[i][j] );
                        if( j >= D3MSA3.msaMap[i].length ){
                            D3MSA3.msaMax[ i ]
                                = D3MSA3.msaMap[i][ D3MSA3.msaMap[i].length-1 ];
                        } else {
                            D3MSA3.msaMax[ i ]
                                = D3MSA3.msaMap[i][j];
                        }
        ...
        for( var i = 0; i <D3MSA3.msaHead.length; i++ ){         
            ...
            _seqHGAT= _seqHG.append("text")    // HGA for link                
                ...
                //.style( "fill", "blue")
                //.attr("text-decoration","underline")
                .text( D3MSA3.config.taxname[D3MSA3.msaHead[i][2]] );
            _seqHGAT
    ...
    renderAA() {
        ...
        for( var i = 0; i < D3MSA3.rectPos.length; i++ ){
            D3MSA3.msaVon.push( D3MSA3.rectPos[i] < -D3MSA3.aaStep ||
                               D3MSA3.rectPos[i] > D3MSA3._msaW  ? false : true);
        }

        for( var i = 0; i < D3MSA3.msaPos.length; i++ ){
            if( D3MSA3.msaVon[i] ){  // add AAs
                for( var j = 0; j < D3MSA3.msaHead.length; j++ ){
                    if( D3MSA3.rectPosOld[i] <=-10
                        || D3MSA3.rectPosOld[i] > D3MSA3._msaW) continue;
                    // box
                    
                    var aa =  D3MSA3.msaSeq[j][i];
                    
                    d3.select( "#seqSeq" + j.toString() +"bk")
                        .append( "rect" )                
                        .attr( "i", i )
                        .attr( "x", D3MSA3.rectPosOld[i] )
                        .attr( "y", -10 )
                        .attr( "width", D3MSA3.aaStepOld )
                        .attr( "height", 10 )
                        .attr( "class", "msa-rect")
                        .style( "fill", D3MSA3._palette1[ aa ] )
                        .attr( "fill-opacity", D3MSA3.aaStep > 16 ? 0.5 : 1 )                        
                        .append("title").text(aa + (D3MSA3.msaMap[j][i]));
                    ...
                    d3.select( "#seqSeq" + j.toString() +"aa")
                        ...
                        .style( "fill", D3MSA3._palette1[ aa ] )
                        .attr( "text-anchor", "middle") 
                        .attr( "x", D3MSA3.msaPosOld[i] )
                        .attr( "y", 0 )
                        .attr( "i", i )
                        .attr( "fill-opacity", D3MSA3.aaStep > 16 ? 1 : 0 )
                        .text( aa )
                        .append("title").text(aa + (D3MSA3.msaMap[j][i]));                   
                }                        
            } 
        ...
        for( var i = 0; i <D3MSA3.msaHead.length; i++ ){
            for( var j = 0; j < D3MSA3.msaSeq[i].length; j++){
                d3.select( "#seqSeq" + i.toString() +"bk rect[i='"+j+"']")
                    .attr( "fill-opacity", D3MSA3.msaOpa)
                    .attr( "x", D3MSA3.rectPos[j] )
                    .attr( "width", D3MSA3.aaStep );
            }
        }
        ...
        for( var i = 0; i <D3MSA3.msaHead.length; i++ ){
            for( var j = 0; j < D3MSA3.msaSeq[i].length; j++){            
                d3.select( "#seqSeq" + i.toString() +"aa text[i='"+j+"']")
                    .attr( "x", D3MSA3.msaPos[j])
                    .attr( "fill-opacity", function (d) {
                        if( D3MSA3.aaStep > 16 ){
                            return 1;
                        } else {
                            var op = -1 + 2*D3MSA3.aaStep/16;
                            if(op < 0){
                                return 0;
                            } else {
                                return op;
    ...
    updateAA() {
        ...
        //console.log(D3MSA3.msaNewFrame);
        //console.log(":"+D3MSA3.aaStep+":"+D3MSA3.aaOffset+":");
        
        for( var i = 0; i < D3MSA3.rectPos.length; i++ ){
            msaVonCurrent.push( D3MSA3.rectPos[i] < -D3MSA3.aaStep  ||
                                D3MSA3.rectPos[i] > D3MSA3._msaW  ? false:true);        
        }

        for( var i = 0; i < D3MSA3.rectPos.length; i++ ){
            if( oonmin < 0 &&  D3MSA3.msaVon[i]) oonmin = i;
            if(  D3MSA3.msaVon[i] ) oonmax = i;
        }

        var smin = D3MSA3.msaNewFrame.min;
        var joff = D3MSA3.msaNewFrame.min;
        
        if( oonmin < smin){
            smin = oonmin;
            joff = oonmin;
        }
        var smax = D3MSA3.msaNewFrame.max;
        if( oonmax >smax)  smax = oonmax;
        ...
        for( j = smin; j <= smax; j++ ){
            ...
            cur = (j >= D3MSA3.msaNewFrame.min &&
                   j <= D3MSA3.msaNewFrame.max) ? true : false;
            old = (j >= oonmin && j <= oonmax) ? true : false;
            ...
            if( cur != old ){ 
                if( cur ){ // add AA/box
                    for( var i = 0; i < D3MSA3.msaHead.length; i++ ){
                        ...
                        var aa =  D3MSA3.msaSeq[i][j];
                        
                        d3.select( "#seqSeq" + i.toString() +"bk")  // box
                            .append( "rect" )                
                            .attr( "i", j )
                            .attr( "x", D3MSA3.rectPosOld[j] )
                            //.attr( "x", D3MSA3.msaNewFrame.rectPos[j] )
                            .attr( "y", -10 )
                            .attr( "width", D3MSA3.aaStepOld )
                            .attr( "height", 10 )
                            ...
                            .style( "fill", D3MSA3._palette1[ aa ] )
                            .attr( "fill-opacity", D3MSA3.aaStep > 16 ? 0.5:1 )
                            .append("title").text(aa+(D3MSA3.msaMap[i][j+joff]));
                        ...
                        d3.select( "#seqSeq" + i.toString() +"aa") // letter 
                            ...
                            .style( "fill", D3MSA3._palette1[ aa ] )
                            .attr( "text-anchor", "middle") 
                            .attr( "x", D3MSA3.msaPosOld[j] )
                            .attr( "y", 0 )
                            .attr( "i", j )
                            .attr( "fill-opacity", D3MSA3.aaStep > 16 ? 1 : 0 )
                            .text( aa )
                            .append("title").text(aa + (D3MSA3.msaMap[i][j]));
                    }                        
        ...
        D3MSA3.msaVon = msaVonCurrent;
        
        for( var i = 0; i <D3MSA3.msaHead.length; i++ ){
            for( var j = 0; j < D3MSA3.msaNewFrame.msaPos.length; j++){
                
                d3.select( "#seqSeq" + i.toString() +"bk " +
                           "rect[i='"+(j+D3MSA3.msaNewFrame.min)+"']")
                    .attr( "fill-opacity", D3MSA3.msaOpa)
                    .attr( "x", D3MSA3.msaNewFrame.rectPos[j] )
                    .attr( "width", D3MSA3.aaStep );                    
            }
        ...
        for( var i = 0; i <D3MSA3.msaHead.length; i++ ){
            for( var j = 0; j < D3MSA3.msaNewFrame.msaPos.length; j++){            
                d3.select( "#seqSeq" + i.toString() +"aa " +
                           " text[i='"+(j+D3MSA3.msaNewFrame.min) + "']")
                    .attr( "x", D3MSA3.msaNewFrame.msaPos[j])
                    .attr( "fill-opacity", function (d) {
                        if( D3MSA3.aaStep > 16 ){
                            return 1;
                        } else {
    ...
    setSelectMap: function( smap ){
        //console.log(smap);
        for( pc of D3MSA3.select ){ // go over existing elements
            //console.log("sel:",pc);
            if( pc in Object.keys(smap) ){ // no selection change, remove from smap                
                smap.delete(pc);                 
            } else { // drop selection
                D3MSA3.select[pc].remove(); // drop rectangle
                delete D3MSA3.select[pc];  // drop from map   
            }
        }
        ...
        for( pc of Object.keys(smap) ){
            ...
            //console.log(pc, smap[pc]);
            pcl =pc.split(":");
            D3MSA3.select[pc] =  D3MSA3.addSelect( Number(pcl[0]),
                                                 pcl[1],
                                                 smap[pc].name);            
        }
    },

    setSelectList: function( slist, rseq  ){
        
        for( pc in D3MSA3.select ){ // go over existing elements
            if( pc in slist ){ // no selection change, remove from slist
        ...
        for( i = 0 ; i < slist.length; i++){
            pcl = slist[i].split(":");
            D3MSA3.select[ slist[i] ] =  D3MSA3.addSelect(Number(pcl[0]),pcl[1],pcl[0]);            
        }
    ...
    updateSelect:  function(){
        ...
        for( k in D3MSA3.select ){
            kl = k.split(":");
            
            var mp = D3MSA3.msaRMap[ Number( kl[0] ) ];
            var pos = mp * D3MSA3.aaStep - D3MSA3.aaOffset;
    ...
    addSelect: function( pos, color, name ){
        try{
            var mp = D3MSA3.msaRMap[pos];
            var x = mp * D3MSA3.aaStep - D3MSA3.aaOffset;
            ...
            var rect =  d3.select( "#seqMSA_select" )
                ...
                .attr( "y", -2 )
                .attr( "width", D3MSA3.aaStep )
                .attr( "height", 14*(D3MSA3.msaSeq.length + 0.5) )
                .attr( "class", "msa-select");
