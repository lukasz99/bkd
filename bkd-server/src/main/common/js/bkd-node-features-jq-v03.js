...
BKDnodeFeatures = {
    ...
    init: function( tgt, format, data, myurl ){
        ...
        for( var i =0; i < data.attrs.length;i++ ){
            ...
            if( catt.cvType.name == 'mane-sequence'){
                iseq.mseq = {"seq":catt.value };
                if( catt.xrefs != undefined ){
                    for( var j=0; j< catt.xrefs.length; j++){
                        iseq.mseq[ catt.xrefs[j]['ns'] ] = catt.xrefs[j]['ac'];                       
                    }
                }
            }
            if( catt.cvType.name == 'canonical-sequence'){
                iseq.cseq = {"seq":catt.value };
                if( catt.xrefs != undefined ){
                    for( var j=0; j< catt.xrefs.length; j++){
                        iseq.cseq[ catt.xrefs[j]['ns'] ] = catt.xrefs[j]['ac'];                       
                    }
                }
            }
            if( catt.cvType.name == 'alternate-sequence'){
                var aseq = {"seq":catt.value };
                if( catt.xrefs != undefined ){
                    for( var j=0; j< catt.xrefs.length; j++){
                        aseq[ catt.xrefs[j]['ns'] ] = catt.xrefs[j]['ac'];                       
                    }
                }
                iseq.aseq.push(aseq);
            }
        ...
        $('#flist-source #iseq').append('<option value="canonical">Canonical('
                                        + iseq.cseq.upr           
                                        +')</option>');

        $('#flist-source #iseq').append('<option value="mane">MANE-Select('
                                        + iseq.mseq.upr
                                        +')</option>');

        $('#flist-source #iseq').on('change', function() {
            alert( this.value );
        });
