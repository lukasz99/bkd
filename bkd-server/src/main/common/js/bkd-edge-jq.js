console.log("bkd-edge-jq: common");
...
BKDedge = {
  ...
  init: function( data, srcAnchor, edgeAnchor, flist, mode){
    BKDedge.view( data, srcAnchor, edgeAnchor, flist, mode);
  },

  view: function(data, srcAnchor, edgeAnchor, flist, mode){
    ...
    if(data == null ){
      $( edgeAnchor ).hide();            
      BKDedge.search( data, this.srcAnchor );
      $( srcAnchor ).show();            
    }else{
      $( srcAnchor ).hide();            
      BKDedge.edgeView( data, this.edgeAnchor, flist, mode );
      $( edgeAnchor ).show();
    }
  },
           ...
           encode: true,}).done(
             function (data) {
               if( data.rdata != null && data.rdata.length > 0){
                 BKDedge.searchView( data.rdata, BKDrep.srcAnchor, "view" );  
    ...
    if( format.pane != null && format.pane.length > 0){  
       ...
       for(var i=0; i< format.pane.length; i++ ){
          ...
          $("#bkd-sb-"+cid).mouseover( function(event){
                 ...
                 //if(event.currentTarget.id == BKDedge.paneon ){
          ...
          $("#bkd-sb-"+cid).click( function(event){
                 $("#bkd-nv-" + BKDedge.paneon).hide();
                 $("#bkd-sb-" + BKDedge.paneon).addClass("bkd-sb-entry-off");
                 $("#bkd-sb-" + BKDedge.paneon).removeClass("bkd-sb-entry-on");
                 BKDedge.paneon = event.currentTarget.id.replace("bkd-sb-","");
                 $("#bkd-nv-" + BKDedge.paneon).show();
                 $("#bkd-sb-" + BKDedge.paneon).addClass("bkd-sb-entry-on");
                 $("#bkd-sb-" + BKDedge.paneon).removeClass("bkd-sb-entry-off");                 
               });
          ...
          // select default pane
          //--------------------
          if( cid == BKDedge.paneon ){  // select default pane
             $( "#bkd-nv-" + cid ).show();
