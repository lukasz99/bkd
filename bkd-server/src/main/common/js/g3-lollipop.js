...
function Lollipop(target, chartType, width) {
    ...
    var sequenceOpt = {
        ...
        text: {
            ...
            palette: { "A":"LimeGreen", "C":"orange", "D":"red",   "E":"red",
                       "F":"LimeGreen", "G":"LimeGreen",  "H":"blue",  "I":"LimeGreen",
                       "K":"blue",  "L":"LimeGreen",  "M":"LimeGreen", "N":"grey",
                       "P":"LimeGreen", "Q":"grey",   "R":"blue",  "S":"grey",
    ...
    var _drawSequence = function () {
        ...
        // sequence
        _sequence = _sequenceViz.append("g")
            .attr("class", sequenceOpt.sequenceClassName.group);

        // draw sequence
        _seqAAs = _sequence.append("g")
            .attr("clip-path", "url(#" + sequenceOpt.defsId+ ")")
            .selectAll(sequenceOpt.sequenceClassName.group)
            .data(seqData).enter()
            .append("text")             
            .attr("class", sequenceOpt.sequenceClassName.text)
            .style("font", sequenceOpt.text.font)
            .style("fill", function (d) {
                //console.log("D: " + String(d.aa));
                return sequenceOpt.text.palette[d.aa];
            })
    ...
    var _popMouseOver = function (event, d, target) {
        if (options.tooltip) {
            _popTooltip.show(d, target);
        }
    };

    var _popMouseOut = function (event,d) {
        if (options.tooltip) {
            _popTooltip.hide(d);
        }
    };
    ...
    var _popClick = function (event,d, parentG) {
        ...
        if (d._currentState.showLabel) {
            ...
            var txtHolder = parentG
                ...
                .style("fill", lollipopOpt.popLabel.color)
                .style("font", _font)
                .attr("dy", ".35em")
                .attr("text-anchor", "end");
    ...
    var _addLollipopLegend = function () {
        ...
        for (let _d in _currentStates) {
            _lollipopLegend.addSeries({
                "key": _d,
                "value": {
                    fill: lollipopOpt.lollipopHook.fcolor[ _d ],
        ...
        _lollipopLegend.dispatch.on("legendClick", function (key, selected) {
            ...
            // NOTE: LS: legend handler
            
            if( lollipopOpt.legendHook != null ){                
                lollipopOpt.legendHook.action( key, selected,
                                               lollipopOpt.legendHook.options );
            }                      
        });
    ...
    var _drawPie = function (d) {
        ...
        _pieSlice.selectAll(lollipopOpt.popClassName.slice)
            ...
            .attr("fill", function (d) {
                //console.log(d.data.key, JSON.stringify(lollipopOpt.lollipopHook.fcolor));
                return lollipopOpt.lollipopHook.fcolor[d.data.key];
        ...
        _chartG.on("mouseover", function (event, d) { _popMouseOver(event,d); })
            .on("mouseout", function (event,d) { _popMouseOut(event, d); })
            .on("click", function (event,d) { _popClick(event, d, _chartG); });
    };
    ...
    var _calcOptions = function () {
        ...
        if( options.seq ){
            _svgH = _svgH + sequenceOpt.height;
        }
        _svgW = width;
...
//        _yValueMax = _yRange[1] = _getYMaxAfterNice(_yRange[1] * lollipopOpt.yPaddingRatio);
    ...
    var _calcAxis = function () {
        ...
        _yAxis = d3.axisLeft(_yScale).tickSize(-_mainW).ticks(3).tickFormat(d3.format("d"));
...
/* */
    ...
    var _initViz = function () {
        ...
        // sequence viz
        _sequenceViz = _viz.append("g").attr("id", sequenceOpt.id)
            .attr("transform", "translate(0, " + (sequenceOpt.margin.top) + ")");
        _initSequenceViz();
        ...
        if( options.seq){
           _mainViz.attr("transform", "translate(0, " + ( _seqH ) + ")");
        }
        _initMainViz();
        ...
        if(options.seq){
           _domainViz = _viz.append("g").attr("id", domainOpt.id)
               .attr("transform", "translate(0, " + (_mainH + domainOpt.margin.top + _seqH + sequenceOpt.margin.top ) + ")");
           _initDomainViz();
        } else {
           _domainViz = _viz.append("g").attr("id", domainOpt.id)
    ...
    lollipop.options = {
        ...
        get legendItems() { return _lollipopLegend.series; },
        ...
        set seq(_) { options.seq = _; }, 
        get seq() { return options.seq; }, 

        set lollipopHook(_) { lollipopOpt.lollipopHook = _; }, 
        get lollipopHook() { return lollipopOpt.lollipopHook; }, 

        set legendHook(_) { lollipopOpt.legendHook = _; }, 
        get legendHook() { return lollipopOpt.legendHook; }, 
