
<!-- common-js.jsp:BEG -->
<script src="jq/jquery-3.6.3.js" type="text/javascript" language="JavaScript"></script>
<script src="jq/ui/1.13.2-overcast/jquery-ui.js" type="text/javascript" language="JavaScript"></script>
<script src="ctbl/datatables.js" type="text/javascript" language="JavaScript"></script>
<script src="js/bkd-modal.js" type="text/javascript" language="JavaScript"></script>
<script src="js/bkd-usermgr-jq.js" type="text/javascript" language="JavaScript"></script>
<script src="js/bkd-useredit-jq.js" type="text/javascript" language="JavaScript"></script>
<script src="js/bkd-userprefmgr-jq.js" type="text/javascript" language="JavaScript"></script>
<script src="js/bkd-stat.js" type="text/javascript" language="JavaScript"></script>
<!-- common-js.jsp:END -->
