...
@Entity
@DiscriminatorValue("nodefeatrep")
public class NodeFeatureReport extends Report{

    
    @ManyToOne(fetch = FetchType.EAGER, cascade = CascadeType.ALL)
    @JoinColumn(name="fk_feat", nullable=false)
    private NodeFeat feature;
    
    public void setFeature(NodeFeat feature){
        this.feature = feature;
    }
    
    public NodeFeat getFeature(){
        return this.feature;
    }
    ...
    @Column(name="fk_feat", insertable=false, updatable=false)
    long feature_id;
    ...
    public static NodeFeatureReport fromJsonForm( String form,
                                                  CvTerm freportType,
                                                  Source source ){

        Logger log = LogManager.getLogger( FeatureReport.class );

        log.info( "NodeFeatureReport: fromJsonForm..." );
        ...
        try{
            ...
            for( Iterator<String> k = jform.keys(); k.hasNext(); ){
                ...
                log.info( "fromJsonForm -> KEY: " + key + "  |||| VAL: " + val );
    ...
    public Map<String, Object> toMap( ProteinNode tgt ){
        ...
        if( this.cvtype != null){
            map.put( "type", this.cvtype.toMap());
        }
        ...
        if(this.getComment().length() > 0){
            map.put("comments",this.getComment());
        }

        
        // parse json fields (map of ns/ac/value fields)
        //----------------------------------------------

        // "channel-inactivation":{"ac":"bkd:0004","ns":"bkd","value":"No Change"}
        
        try {
            JSONObject fjo = null;
            fjo = new JSONObject( this.getJval() );                
                        
            for( Iterator<String> k = fjo.keys(); k.hasNext(); ){
                String key = k.next();
                Map<String, Object> jmap = new HashMap<String,Object>();
                JSONObject jo = fjo.getJSONObject( key );
                if( jo.has("ns") ){
                    jmap.put("ns", jo.getString( "ns" ) );
                }
                if( jo.has("ac") ){
                    jmap.put("ac", jo.getString( "ac" ) );
                }
                if( jo.has("value") ){
                    jmap.put( "value", jo.getString( "value" ) ) ;
                }
        ...
        tmap.put( "feature", this.getFeature().toMap() );
                
   
        if( tgt == null ) return map;
        ...
        tmap.put( "ns", tgt.getNs() );
        tmap.put( "ac", tgt.getAc() );
        tmap.put( "upr", tgt.getUpr() );
        tmap.put( "label", tgt.getLabel() );
        tmap.put( "name", tgt.getName() );
    ...
    public String toString(){
        ...
        //}
        return "NodeFeatureReport: toString";
    }
